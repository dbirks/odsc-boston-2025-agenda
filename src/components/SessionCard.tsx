          ...
          <CardContent className="px-3 sm:px-6 pt-0 pb-3">
            ...
            {(session.webinarLink || session.slackUrl || session.replayLink || session.detailLink || session.prerequisite) && (
              <div className="mt-3 flex flex-col gap-1 sm:gap-2">
                <h4 className="text-xs sm:text-sm font-medium mb-1 tracking-normal">Session Links</h4>
                <div className="flex flex-col gap-1">
                  {session.webinarLink && (
                    <a 
                      ...
                      className="text-sm text-blue-600 hover:underline flex items-center tracking-normal"
                    >
            ...
            <div className="space-y-3 mt-3">
              ...
              <div className="border-b pb-1 sm:pb-2">
                <h4 className="text-xs sm:text-sm font-medium mb-1 sm:mb-2 tracking-normal">Access Levels</h4>
                <div className="flex flex-wrap gap-0.5 sm:gap-1">
                  ...
                  {session.ticketTypes && session.ticketTypes.length > 0 ? (
                    session.ticketTypes.map((ticketType: string, index: number) => (
                      <Badge 
                        key={index}
                        className={`${passedClasses.badge} text-xs tracking-normal`}
                        variant={
              ...
              <div className="border-b pb-1 sm:pb-2">
                <h4 className="text-xs sm:text-sm font-medium mb-1 sm:mb-2 tracking-normal">Session Details</h4>
                <div className="flex flex-wrap gap-0.5 sm:gap-1">
                  {/* Session type */}
                  {session.subtrack && (
                    <Badge variant="outline" className="bg-gray-50 tracking-normal">{session.subtrack}</Badge>
                  )}
                  
                  {/* Session difficulty */}
                  <Badge variant="outline" className="bg-gray-50 tracking-normal">{session.difficulty}</Badge>
                  
                  {/* Unlockable indicator */}
                  {session.unlockable && (
                    <Badge variant="outline" className="bg-amber-50 border-amber-300 text-amber-700 tracking-normal">
                      Unlockable
                    </Badge>
                  )}
                </div>
              ...
              {tagArray.length > 0 && (
                <div>
                  <h4 className="text-xs sm:text-sm font-medium mb-1 sm:mb-2 tracking-normal">Topics</h4>
                  <div className="flex flex-wrap gap-0.5 sm:gap-1">
                    {tagArray.map((tag: string | undefined, index: number) => tag && (
                      <Badge key={index} variant="outline" className="tracking-normal">{tag}</Badge>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="flex justify-between items-center">
            <div className="flex-grow overflow-hidden">
              ...
              <div className="flex flex-wrap items-center gap-x-1 gap-y-0.5 text-xs text-gray-500 mt-1 w-full tracking-normal">
                ...
                <span className="text-xs mx-0.5 hidden sm:inline">•</span>
                <span className="text-xs mx-0.5 sm:hidden">/</span>
                <span className="whitespace-nowrap tracking-normal">{session.duration} min</span>
                {session.location && (
                  <>
                    <span className="text-xs mx-0.5 hidden sm:inline">•</span>
                    <span className="text-xs mx-0.5 sm:hidden">/</span>
